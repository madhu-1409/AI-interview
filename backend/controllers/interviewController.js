      ...
      try {
        ...
        if (customQuestions && Array.isArray(customQuestions) && customQuestions.length > 0) {
          ...
          if (questionCount && questions.length < questionCount) {
            ...
            const additionalQuestions = await generateConfigurableQuestions(
              resumeText,
              role,
              remainingCount
            );
          ...
          questions = await generateConfigurableQuestions(
            resumeText,
            role,
            questionCount,
            customQuestions
        ...
        // Error generating configurable questions, using fallback
  ...
  try {
    ...
    if (answerText.trim()) {
      try {
        ...
        // Error grading answer with AI, using fallback
    ...
    if (currentIndex < interviewSession.questions.length - 1) {
      ...
      try {
        ...
        const candidateInTransaction = await Candidate.findById(
          interviewSession.candidateId
        ).session(session);
        if (candidateInTransaction) {
          candidateInTransaction.status = 'completed';
          await candidateInTransaction.save({ session });
        }
      ...
      try {
        ...
        // Error generating summary with AI, using fallback
...
async function deleteInterviewById(req, res, next) {
  try {
    ...
    try {
      ...
      const candidateToDelete = await Candidate.findById(interviewSession.candidateId).session(
        session
      );
      if (candidateToDelete) {
        candidateToDelete.isDeleted = true;
        candidateToDelete.deletedAt = new Date();
        await candidateToDelete.save({ session });
      }
...
async function patchInterviewById(req, res, next) {
  try {
    ...
    try {
      ...
      // Mark candidate as completed if not already
      const candidateToComplete = await Candidate.findById(interviewSession.candidateId).session(
        session
      );
      if (candidateToComplete && candidateToComplete.status !== 'completed') {
        candidateToComplete.status = 'completed';
        await candidateToComplete.save({ session });
      }
...
async function adjustQuestionScore(req, res, next) {
  try {
    ...
    const questionIndex = interviewSession.questions.findIndex(
