  ...
  try {
    ...
    if (sortBy === 'score') {
      ...
      candidatesWithScores.sort((a, b) => {
        return order === 1
          ? a.interviewScore - b.interviewScore
      ...
      const candidatesWithSessions = await Promise.all(
...
function deleteMyData(req, res, next) {
  try {
    ...
    res.json(
      formatSuccessResponse(
        null,
        'Data deletion request received. Your data will be deleted according to our retention policy.'
      )
    );
  } catch (error) {
    next(error);
  ...
  try {
    ...
    if (req.path.includes('/csv')) {
      ...
      const csvRows = candidatesWithSessions.map(candidate => {
        const score =
          candidate.session && candidate.session.score ? candidate.session.score : 'N/A';
        const questionsAnswered =
          candidate.session && candidate.session.questions
            ? candidate.session.questions.filter(q => q.answer).length
            : 0;
    ...
    return res.json(exportDataResult);
  } catch (error) {
    next(error);
  }
}
