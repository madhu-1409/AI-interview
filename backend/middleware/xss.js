...
const sanitizeInput = (req, res, next) => {
  try {
    ...
    if (req.headers && typeof req.headers === 'object') {
      ...
      for (let i = 0; i < sensitiveHeaders.length; i += 1) {
    ...
    if (req.body && typeof req.body === 'object') {
      ...
      for (let i = 0; i < keys.length; i += 1) {
        ...
        if (key.includes('phone') && typeof req.body[key] === 'string') {
          if (
            req.body[key] &&
            !validator.isMobilePhone(req.body[key], 'any', { strictMode: false })
          ) {
            return next(
              new AppError(`Invalid phone number format for field: ${key}`, 400, 'VALID_004')
            );
          }
        }
      }
      ...
      if (source && typeof source === 'object') {
        ...
        for (let j = 0; j < keys.length; j += 1) {
          ...
          if (typeof value === 'string') {
            ...
            for (let k = 0; k < sqlInjectionPatterns.length; k += 1) {
        ...
        for (let j = 0; j < keys.length; j += 1) {
          ...
          if (typeof value === 'string') {
            ...
            if (value.length > maxLength) {
              return next(
                new AppError(
                  `Input for ${key} exceeds maximum length of ${maxLength} characters`,
                  400,
                  'SAN_005'
                )
              );
